 ...
 * @param options - 옵션 객체
 * @param options.count - 몇 번째 영업일인지 (기본값: 1)
 ...
 * getNextBusinessDay('2024-01-01', { count: 2 }); // '2024-01-03' (신정 다음 두 번째 영업일)
 * getNextBusinessDay('01/01/2024', { count: 1, format: 'MM/dd/yyyy' }); // '01/02/2024' (미국식 포맷 입출력)
 * getNextBusinessDay('2024-01-05', { count: 3 }); // '2024-01-10' (금요일 다음 세 번째 영업일)
 * getNextBusinessDay('2024-12-31', { count: 5 }); // '2025-01-08' (연말연시 다음 다섯 번째 영업일)
 */
export const getNextBusinessDay = (
  ...
  return findNextDate(dateStr, isBusinessDay, { count, format });
};
 ...
 * getPreviousBusinessDay('2024-01-08', { count: 2 }); // '2024-01-04' (월요일 이전 두 번째 영업일)
 * getPreviousBusinessDay('04/03/2024', { count: 3, format: 'MM/dd/yyyy' }); // '02/27/2024' (미국식 포맷 입출력)
 * getPreviousBusinessDay('2024-03-04', { count: 3 }); // '2024-02-27' (삼일절 대체휴일 이전 세 번째 영업일)
 * getPreviousBusinessDay('2024-05-07', { count: 5 }); // '2024-04-30' (어린이날 연휴 이전 다섯 번째 영업일)
 */
export const getPreviousBusinessDay = (
  ...
  const { count = 1, format = "yyyy-MM-dd" } = options;
  return findPreviousDate(dateStr, isBusinessDay, { count, format });
};
 ...
 * getLastBusinessDay('01/10/2026', { format: 'MM/dd/yyyy' }); // '01/09/2026' (미국식 포맷 입출력)
