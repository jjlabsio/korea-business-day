...
describe("business", () => {
  ...
  describe("getNextBusinessDay", () => {
    ...
    describe("with count parameter", () => {
      it("count=1일 때 기본 동작과 동일해야 함", () => {
        expect(getNextBusinessDay("2025-08-25", 1)).toEqual("2025-08-26"); // 월요일 → 화요일
        expect(getNextBusinessDay("2025-01-01", 1)).toEqual("2025-01-02"); // 신정 → 목요일
      });

      it("count=2일 때 두 번째 영업일을 반환해야 함", () => {
        expect(getNextBusinessDay("2025-08-25", 2)).toEqual("2025-08-27"); // 월요일 → 수요일
        expect(getNextBusinessDay("2025-08-29", 2)).toEqual("2025-09-02"); // 금요일 → 화요일 (주말 건너뜀)
      });

      it("count=3일 때 세 번째 영업일을 반환해야 함", () => {
        expect(getNextBusinessDay("2025-08-25", 3)).toEqual("2025-08-28"); // 월요일 → 목요일
        expect(getNextBusinessDay("2025-08-29", 3)).toEqual("2025-09-03"); // 금요일 → 수요일
      });

      it("count=5일 때 다섯 번째 영업일을 반환해야 함", () => {
        expect(getNextBusinessDay("2025-08-25", 5)).toEqual("2025-09-01"); // 월요일 → 다음주 월요일
        expect(getNextBusinessDay("2025-01-01", 5)).toEqual("2025-01-08"); // 신정 → 5번째 영업일
      });

      it("연휴 기간에서 count를 사용해야 함", () => {
        expect(getNextBusinessDay("2025-01-27", 2)).toEqual("2025-02-03"); // 임시공휴일 → 설 연휴 후 두 번째 영업일
        expect(getNextBusinessDay("2025-01-27", 3)).toEqual("2025-02-04"); // 임시공휴일 → 설 연휴 후 세 번째 영업일
      });

      it("잘못된 count 값에 대해 에러를 발생시켜야 함", () => {
        expect(() => getNextBusinessDay("2025-08-25", 0)).toThrow(
          "count must be a positive number"
        );
        expect(() => getNextBusinessDay("2025-08-25", -1)).toThrow(
          "count must be a positive number"
        );
  ...
  describe("getPreviousBusinessDay", () => {
    ...
    describe("with count parameter", () => {
      it("count=1일 때 기본 동작과 동일해야 함", () => {
        expect(getPreviousBusinessDay("2025-08-26", 1)).toEqual("2025-08-25"); // 화요일 → 월요일
        expect(getPreviousBusinessDay("2025-01-02", 1)).toEqual("2024-12-31"); // 신정 다음 날 → 전년 마지막 영업일
      });

      it("count=2일 때 두 번째 이전 영업일을 반환해야 함", () => {
        expect(getPreviousBusinessDay("2025-08-27", 2)).toEqual("2025-08-25"); // 수요일 → 월요일
        expect(getPreviousBusinessDay("2025-08-25", 2)).toEqual("2025-08-21"); // 월요일 → 목요일 (주말 건너뜀)
      });

      it("count=3일 때 세 번째 이전 영업일을 반환해야 함", () => {
        expect(getPreviousBusinessDay("2025-08-28", 3)).toEqual("2025-08-25"); // 목요일 → 월요일
        expect(getPreviousBusinessDay("2025-08-25", 3)).toEqual("2025-08-20"); // 월요일 → 수요일
      });

      it("count=5일 때 다섯 번째 이전 영업일을 반환해야 함", () => {
        expect(getPreviousBusinessDay("2025-09-01", 5)).toEqual("2025-08-25"); // 월요일 → 전주 월요일
        expect(getPreviousBusinessDay("2025-01-08", 5)).toEqual("2024-12-31"); // 5번째 이전 영업일 (연도 넘어감)
      });

      it("연휴 기간에서 count를 사용해야 함", () => {
        expect(getPreviousBusinessDay("2025-01-31", 2)).toEqual("2025-01-23"); // 설 연휴 후 첫 영업일 → 두 번째 이전 영업일
        expect(getPreviousBusinessDay("2025-01-31", 3)).toEqual("2025-01-22"); // 세 번째 이전 영업일
      });

      it("잘못된 count 값에 대해 에러를 발생시켜야 함", () => {
        expect(() => getPreviousBusinessDay("2025-08-25", 0)).toThrow(
          "count must be a positive number"
        );
        expect(() => getPreviousBusinessDay("2025-08-25", -1)).toThrow(
          "count must be a positive number"
        );
        expect(() => getPreviousBusinessDay("2025-08-25", -5)).toThrow(
          "count must be a positive number"
        );
      });
    });
